import { AnySQLiteColumn, check, foreignKey, integer, primaryKey, sqliteTable, text } from "drizzle-orm/sqlite-core";
...
export const collection = sqliteTable("collection", {
    ...
    primaryKey({
        columns: [table.playerId, table.expansionCode, table.cardNumber],
        name: "collection_player_id_expansion_code_card_number_pk",
    }),
    check("card_check_1", sql`card_number >= 0`),
    check("card_check_2", sql`rarity IN ('♢', '♢♢', '♢♢♢', '♢♢♢♢', '☆'`),
    check("collection_check_3", sql`card_number >= 0`),
